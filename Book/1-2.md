...
1. `<script>` 标签不需要包含 `type="text/javascript"`，因为脚本默认的就是 JavaScript。
...
6. 数组:
 ...
 在  JavaScript 中，所有的变量是基上都是某种类型的对象，你感受下。理想情况下爱，你不应该改修改 Array 对象的属性， 而应该使用通用的对象（object）。
